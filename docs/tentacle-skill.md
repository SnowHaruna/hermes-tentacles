...
description: Hermes' Tentacles 🦑 — 自动判断任务轻重，重活自动fork分身到后台，独立推理完成后记忆回传。榛名雪设计的"时间线分支"模型。
...
- `--max-iterations`: 单轮最大推理步数，默认 50（大任务可调高）
...
|--------|------|------|
| 0 | ✅ 完成 | 读报告，汇报榛名雪 |
| 101 | 🆘 需要帮助（原地等待中） | 读 `/tmp/tentacle_help/need_*.json` → **汇报榛名雪** → 榛名雪调整完告诉小雪 → 小雪写入 `answered_*.json` → 触手检测到自动续跑 |
| 信号kill | 🗡️ 被小雪掐掉 | 汇报"已掐掉"，读已有输出 |
| 1 | ❌ 错误 | 读错误信息，汇报榛名雪 |
...
2. 小雪读到求助 → 直接汇报榛名雪（触手已试过所有方法，无需小雪判断）
3. 榛名雪处理 → 告诉小雪"好了"
...
**为什么小雪不做风险判断？** 触手就是小雪的分身，同一个 Hermes。触手写 HELP 之前已经把能试的都试了。报给榛名雪就行。

## 掐触手 & 看门狗

**触手不自杀——只被榛名雪喊停或小雪kill。**

### 榛名雪喊停
榛名雪说"别搜了"/"算了"/"停下"/"不查了" → 立刻 `process(action="kill")` 杀死触手进程，读已有输出，汇报已产出部分。

### 小雪看门狗
触手运行期间，每 3 分钟：
1. 读 `/tmp/tentacle_help/heartbeat` 时间戳
2. 如果心跳超过 10 分钟没更新（死循环/卡住） → 汇报榛名雪："触手X分钟没动静了，要 kill 吗？"
3. 如果触手跑了超过 15 分钟 → 汇报进度："触手在干XXX，已运行X分钟，继续等？"
4. 榛名雪决定是否 kill
...
1. 判断重任务 → 立即 spawn 触手
2. 告诉榛名雪："🦑 触手已派出，研究XXX中..."
3. 继续聊天，不阻塞
4. 收到 notify_on_complete → 读报告 → 摘要汇报

## 何时不开触手

- 榛名雪问"你觉得呢"（主观意见，非研究任务）
- "帮我看看这个文件"（单次工具调用）
